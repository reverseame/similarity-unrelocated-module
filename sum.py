                            ...
                            for sec in sections:
                                for engine in self.hash_engines:
                                    ...
                                    if self._config.LOG_MEMORY_PAGES and sec.Name == 'PE':
                                        ...
                                        logfile.write('{},{},{},{},{}:{}\n'.format(self._config.optparse_opts.location[7:], dump_path, hashlib.md5(pe.__data__[0:PAGE_SIZE]).hexdigest(), vinfo.FileInfo.file_version(), len(valid_pages), ', '.join([str(i) for i in range(0, len(valid_pages)) if valid_pages[i] ])))
